...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <a href="{{ route('admin.ras-hewan.edit', ['id' => $ras->idras_hewan]) }}" class="btn-edit">✏ Edit</a>
                <form action="{{ route('admin.ras-hewan.destroy') }}" method="POST" style="display:inline;">
                    @csrf
                    <input type="hidden" name="id" value="{{ $ras->idras_hewan }}">
                    <button type="submit" class="btn-delete" onclick="return confirm('Yakin hapus?')">🗑 Hapus</button>
                </form>
            </td>
        </tr>
        @empty
        <tr><td colspan="4">Tidak ada data ras hewan.</td></tr>
        @endforelse
    </tbody>
</table>
